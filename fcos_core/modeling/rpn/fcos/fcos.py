...
class FCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.norm_reg_targets = cfg.MODEL.FCOS.NORM_REG_TARGETS      # True: normalizing the regression targets with FPN strides
        self.centerness_on_reg = cfg.MODEL.FCOS.CENTERNESS_ON_REG  #True
        self.use_dcn_in_tower = cfg.MODEL.FCOS.USE_DCN_IN_TOWER #False

        cls_tower = []
        bbox_tower = []
        for i in range(cfg.MODEL.FCOS.NUM_CONVS): # range(4)
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            # 这一句是什么意思呢，为什么只有测试的时候需要乘以stride，
            # 不太明白是什么意思，
            # 另外，看这里的self.norm_reg_targets应该是True的，
            # 那作者就是没有用论文里说的下面else里面的exp（）函数吧，
            # 还有就是论文里作者说回归的都是正数，
            # 所以直接用exp，
            # 可是看这个if 的分支里面是先relu了，然后也没用exp，
            # 不太懂是什么逻辑呢
            ...
            if self.norm_reg_targets:
                ...
                if self.training:
                    ...
                    bbox_reg.append(bbox_pred * self.fpn_strides[l]) # chwangteng: why?
            else:
