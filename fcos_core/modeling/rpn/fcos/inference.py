...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        #centerness torch.Size[1,15200]
        candidate_inds = box_cls > self.pre_nms_thresh
        pre_nms_top_n = candidate_inds.view(N, -1).sum(1)
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)

        # multiply the classification scores with centerness scores
        box_cls = box_cls * centerness[:, :, None] #box_cls torch.Size[1,-1,80]
    ...
    def forward(self, locations, box_cls, box_regression, centerness, image_sizes):
        ...
        """
        sampled_boxes = [] #list(list,list,list,list,list(BoxList,BoxList,BoxList))
        ...
        boxlists = list(zip(*sampled_boxes)) #list(tuple,tuple,tuple(BoxListBoxListBoxListBoxListBoxList))
